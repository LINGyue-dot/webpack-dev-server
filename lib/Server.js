...
class Server {
  ...
  addAdditionalEntries(compiler) {
    ...
    if (this.options.client && isWebTarget) {
      ...
      if (this.options.webSocketServer) {
        ...
        // 关键的几个是 {protocol:ws,hostname:0.0.0.0,port:8080,pathname:'/ws',hot:true}
    ...
    if (typeof webpack.EntryPlugin !== "undefined") {
      for (const additionalEntry of additionalEntries) {
        // TODO 暂时的理解是将 additionalEntry 打包解析 resolve
  ...
  /**
   * @private
   * @returns {Compiler["options"]} webpackConfig
   */
  getCompilerOptions() {
    if (
      // 多编辑器才需要考虑 --> 一般用在多页应用
    ...
    // 返回 webpack(webpackConfig) 的 webpackConfig
  ...
  async normalizeOptions() {
    ...
    // options.open = [{target:"<url>",options:{wait:false}}]
  ...
  /**
   * 返回 WebSocketClient 绝对路径
   * @private
   * @returns {string} require.resolve("../client/clients/WebSocketClient")
   */
  getClientTransport() {
  ...
  /**
   * 返回 require("./servers/WebsocketServer")
   * @private
   * @returns {string} require("./servers/WebsocketServer")
   */
  getServerTransport() {
    let implementation;
  ...
  setupHooks() {
    // 编译无效时候触发
    // tap 方法监听了 invalid hook，然后用 "webpack-dev-server" 作为插件名称，实现了一个回调函数，在编译过程中当发现无效编译时，会触发该回调函数进行处理。
    // webpack-dev-server 用于 logger 使用
    ...
    this.compiler.hooks.done.tap(
      "webpack-dev-server",
      /**
       * @param {Stats | MultiStats} stats 编译的相关信息，如错误、警告、耗时等指标。
       */
      (stats) => {
        if (this.webSocketServer) {
  ...
  createWebSocketServer() {
    ...
    // this.getServerTransport ==> require("./servers/WebsocketServer")
